...
async function getInnerText(el) {
  const elInnerText = await el.getProperty('innerText');
  const elInnerTextValue = elInnerText['_remoteObject'].value;
  return elInnerTextValue;
}
...
describe('Basic user flow for Website', () => {
  ...
  it('Clicking the "Add to Cart" button should change button text', async () => {
    ...
    let buttonTextValue = buttonText['_remoteObject'].value;
    expect(buttonTextValue).toBe('Remove from Cart');

    // Do it again to make sure it's toggled back to "Add to Cart"
    await button.click();
    buttonText = await button.getProperty('innerText');
    buttonTextValue = buttonText['_remoteObject'].value;
  ...
  it('Checking number of items in cart on screen', async () => {
    ...
    const cartCountText = await getInnerText(cartCount);
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    for (const productItem of productItems) {
      ...
      const buttonText = await getInnerText(button);
